...
export async function setMember(request: Request, env: AuthEnv, auth: AuthContext, workspace: WorkspaceAccess): Promise<Response> {
	...
	const existing = await env.DB.prepare("SELECT role FROM workspace_memberships WHERE workspace_id=? AND user_id=?").bind(workspace.id, user.id).first<{ role: WorkspaceRole }>(), now = new Date().toISOString();
	if (!existing) await env.DB.prepare("INSERT INTO workspace_memberships(workspace_id,user_id,role,created_at,updated_at) VALUES(?,?,?,?,?)").bind(workspace.id, user.id, body.role, now, now).run();
	else { const changed = await env.DB.prepare("UPDATE workspace_memberships SET role=?,updated_at=? WHERE workspace_id=? AND user_id=? AND (role<>'OWNER' OR ?='OWNER' OR (SELECT COUNT(*) FROM workspace_memberships WHERE workspace_id=? AND role='OWNER')>1)").bind(body.role, now, workspace.id, user.id, body.role, workspace.id).run(); if (!changed.meta.changes && existing.role === "OWNER" && body.role === "MEMBER") throw new ApiError(409, "CONFLICT", "The final workspace owner cannot be demoted."); if (existing.role !== body.role) await env.DB.prepare("INSERT INTO workspace_audit_events(id,workspace_id,actor_user_id,action,target_user_id,metadata,created_at) VALUES(?,?,?,'MEMBER_ROLE_CHANGED',?,?,?)").bind(crypto.randomUUID(), workspace.id, auth.user.id, user.id, JSON.stringify({ fromRole: existing.role, toRole: body.role }), now).run(); }
	return success({ userId: user.id, role: body.role }, 201);
}
