...
export default function DashboardApp() {
	...
	return <main className="es-app"><header className="es-hero"><div><p className="es-eyebrow">Operations overview</p><h1 className="es-title">Endpoint health</h1><p className="es-subtitle">Live reliability, incident, and delivery state across your workspace.</p></div><div className="es-hero__actions"><button className="es-btn" type="button" onClick={() => { setFormError(null); setForm("new"); }}><span aria-hidden="true">+</span> Add endpoint</button><button className="es-btn es-btn--primary" type="button" onClick={() => void checkAll()} disabled={endpoints.length === 0 || activity.size > 0}><span className="es-btn__status" aria-hidden="true" />Check all</button></div></header>
