...
export default function App() {
  ...
  return <div className="es-shell">
    ...
    <aside className="es-sidebar"><Brand /><p className="es-nav-label">Workspace</p><nav aria-label="Main navigation">
      ...
      {!session.juryMode && workspace.role === 'OWNER' ? <button type="button" onClick={() => setTeamOpen(true)}><Icon name="team" />Team</button> : null}
    ...
    <div className="es-workspace"><header className="es-account-bar">
      <label className="es-workspace-select"><span className="es-nav-label">Workspace</span><select aria-label="Workspace" value={workspaceId} onChange={e => { setWorkspaceId(e.target.value); setActiveWorkspace(e.target.value); setTeamOpen(false); }}>{session.workspaces.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}</select><span className="es-role">{session.juryMode ? 'jury' : workspace.role.toLowerCase()}</span></label>
      <div className="es-account-actions"><ThemeToggle />{session.user ? <><div className="es-account-user"><Avatar name={session.user.githubLogin} url={session.user.avatarUrl} /><span title={session.user.displayName ?? session.user.githubLogin}>@{session.user.githubLogin}</span></div><button className="es-btn es-btn--quiet" disabled={loggingOut} onClick={() => void logout()}>{loggingOut ? 'Signing out…' : 'Sign out'}</button></> : null}</div>
    </header>{error ? <p className="es-shell-error es-form__error" role="alert">{error}</p> : null}<DashboardApp key={workspaceId} juryMode={session.juryMode === true} />
    {teamOpen && workspace.role === 'OWNER' && session.user ? <TeamModal workspace={workspace} currentUserId={session.user.id} onClose={() => setTeamOpen(false)} onMembershipChanged={() => void loadSession(workspaceId, true)} /> : null}</div>
  </div>;
}
